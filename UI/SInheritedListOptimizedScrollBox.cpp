...
void SInheritedListOptimizedScrollBox::Construct(const FArguments& InArgs)
{
	ListOrientation = InArgs._Orientation;
...
void SInheritedListOptimizedScrollBox::SetOrientation(EOrientation InOrientation)
{
	if (ListOrientation == InOrientation)
	{
		return;
	}

	ListOrientation = InOrientation;
	SScrollBox::SetOrientation(InOrientation);
	RequestRefresh();
}

void SInheritedListOptimizedScrollBox::SetEntryExtent(float InEntryExtent)
{
...
void SInheritedListOptimizedScrollBox::SetOverscanCount(int32 InOverscanCount)
{
...
void SInheritedListOptimizedScrollBox::SetItemCount(int32 InItemCount)
{
...
void SInheritedListOptimizedScrollBox::SetVisibleEntries(
...
void SInheritedListOptimizedScrollBox::ClearEntries()
{
	ClearChildren();
}

void SInheritedListOptimizedScrollBox::RequestRefresh()
{
	bRefreshRequested = true;
	Invalidate(EInvalidateWidgetReason::Layout | EInvalidateWidgetReason::Paint);
}

void SInheritedListOptimizedScrollBox::Tick(
	...
	const float InDeltaTime)
{
	...
	const float ViewportExtent = ListOrientation == Orient_Vertical ? LocalSize.Y : LocalSize.X;
	if (ViewportExtent <= 0.0f)
	{
		return;
	}

	const float ScrollOffset = FMath::Max(0.0f, GetScrollOffset());
...
void SInheritedListOptimizedScrollBox::AddExtentSlot(float Extent)
{
	if (ListOrientation == Orient_Vertical)
	{
		AddSlot()
		[
			SNew(SBox)
...
void SInheritedListOptimizedScrollBox::AddEntrySlot(const TSharedRef<SWidget>& Entry)
{
	if (ListOrientation == Orient_Vertical)
	{
		AddSlot()
